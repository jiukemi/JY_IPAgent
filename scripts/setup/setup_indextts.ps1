...
if (-not (Test-Path $venvPy)) { $venvPy = Join-Path $InstallDir "venv\Scripts\python.exe" }
if (Test-Path $venvPy) {
    # Older Gitee mirrors may lack examples_downloader — never fail the whole install on this.
    try {
        & $venvPy -c "from indextts.utils.examples_downloader import ensure_examples_available; ensure_examples_available()"
    } catch {
        Write-Host "!! examples_downloader unavailable (old IndexTTS tree); skip"
    }
    if ($LASTEXITCODE -ne 0) {
        Write-Host "!! examples_downloader failed/missing; will rely on bundled/copied examples if any"
    }
    ...
    $auxPy = Join-Path $PSScriptRoot "_indextts_hf_cache.py"
    if (-not (Test-Path $auxPy)) {
        # Packaged app: script may live next to setup under resources/agent
        $auxPy = Join-Path $Root "scripts\setup\_indextts_hf_cache.py"
    }
    if (Test-Path $auxPy) {
        try {
            & $venvPy $auxPy
        ...
        Write-Host "!! missing _indextts_hf_cache.py — skip pre-download (synth will try repair)"
    }
    Remove-Item Env:INDEXTTS_CKPT_DIR -ErrorAction SilentlyContinue
